...
const ENTITY_NAME_PLURAL = "lists";
const ENTITY_FILE_PATH = "lists/lists.json"
...
router.post('/', function (req, response) {
  ...
  fs.readFile(ENTITY_FILE_PATH, 'utf8', (err, data) => {
    if (err){
      ...
      const newObject = {id:obj[ENTITY_NAME_PLURAL].length+1, name:req.body.name, user:req.body.user, items:req.body.items}
...
router.put('/:id', function (req, response) {
  fs.readFile(ENTITY_FILE_PATH, 'utf8', (err, data) => {
    if (err){
      ...
      const updatedObject =  {id:req.params.id, name:req.body.name, name:req.body.name, user:req.body.user, items:req.body.items};
